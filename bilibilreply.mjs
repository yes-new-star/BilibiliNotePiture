              ...
              for (let i = 0; i < at_nums; i++) {
                ...
                const reply_req = https.request(reply_options, (res) => {
                  ...
                  res.on("end", () => {
                    ...
                    for (let j = 0; j < reply_data.data.seek_root_reply.content.pictures.length; j++) {
                      add_message += `图片${j+1}地址：${reply_data.data.seek_root_reply.content.pictures[j].img_src}\n`;
                    }
                      console.log(
                        "Response body:",
